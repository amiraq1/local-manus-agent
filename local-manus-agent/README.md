...
## Importing local .litertlm models

بدلًا من الاعتماد على إعداد المسارات اليدوي (Set Path)، يوفر لك النظام إمكانية استيراد النماذج مباشرة من جهازك (Import) لسهولة الإدارة.

**الفرق بين Set Path و Import:**
- **Set Path**: يطلب منك تحديد مسار الملف في جهازك. الملف يبقى في مكانه.
- **Import**: يقوم بنسخ الملف (عبر الرفع المجزأ Chunked Upload) إلى مجلد `backend/models/imported` داخل مساحة عمل البرنامج، ويسجل بياناته (SHA256) في `user_config.json`.

**كيف تستورد Gemma E2B؟**
1. يجب أولاً الموافقة على رخصة Google Gemma على موقع Hugging Face.
2. قم بتنزيل ملف `gemma-3n-E2B-it-int4.litertlm` محلياً عبر أداة `huggingface-cli`.
3. افتح Local Manus Agent وانتقل إلى **Settings → Models**.
4. في قسم `Import .litertlm Model`، اختر الملف الذي قمت بتنزيله واضغط على `Import Model`.
5. سيتم رفع الملف تدريجياً. بعد الانتهاء، سيظهر النموذج في قائمة مدير النماذج لتتمكن من اختياره واستخدامه.

**ملاحظات هامة:**
- مسار الحفظ النهائي هو: `backend/models/imported/` (والمؤقت أثناء الرفع في `backend/models/.uploads/`).
- **تحذير Git:** ملفات النماذج ضخمة ولن يتم رفعها إلى GitHub. مجلد `models/` مضاف تلقائياً إلى `.gitignore`. لا تحاول إجبار إضافة النماذج إلى Git.
- استيراد النموذج لا يعني أن `LiteRT-LM runtime` مثبت. إذا ظهرت لك رسالة تفيد بفقدان الـ SDK، فهذا يعني أن الملف موجود ولكن المحرك يحتاج إلى إعداد إضافي.
