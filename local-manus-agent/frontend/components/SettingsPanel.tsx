    ...
    <div className="p-4 space-y-4 overflow-y-auto">
      ...
      {platform?.is_termux && (
        <div className="bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-3">
          <p className="text-yellow-400 text-sm font-medium">📱 Running in Termux Lite Mode</p>
          <ul className="mt-2 text-xs text-yellow-300/80 space-y-1">
            {platform.limitations.map((l, i) => (
              <li key={i}>• {l}</li>
            ))}
          </ul>
          <p className="mt-2 text-[10px] text-dark-400">
            Tip: <code className="text-yellow-400">export OLLAMA_BASE_URL=http://&lt;pc-ip&gt;:11434</code>
          </p>
        </div>
      )}
      ...
      <div className="panel p-3 space-y-2">
        <h3 className="text-sm font-medium text-dark-200">Platform</h3>
        {platform && (
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className={platform.docker_available ? "text-green-400" : "text-dark-500"}>
              {platform.docker_available ? "Available" : "Not available"}
            </div>
            <div className="text-dark-400">Ollama</div>
            <div className={platform.ollama_available ? "text-green-400" : "text-dark-500"}>
      ...
      {/* Links */}
      <div className="panel p-3 space-y-2">
        <h3 className="text-sm font-medium text-dark-200">Links</h3>
        <div className="space-y-1">
          {[
            ...
            <a
              ...
              className="flex items-center gap-2 text-xs text-dark-400 hover:text-primary"
            >
